...
async function restoreDefaultConfig(request: any, apiBase: string) {
...
test.afterEach(async ({ request, apiURL }) => {
  await restoreDefaultConfig(request, apiURL);
});
...
test("config card persists export path and dialog toggle", async ({
  ...
  const persistedConfigResponse = await request.get(`${apiURL}/config`);
  ...
  const preToggleConfigResponse = await request.get(`${apiURL}/config`);
  ...
  const enableResponse = await request.put(`${apiURL}/config`, {
    data: {
      ...baseConfig,
      exportPath,
      showReminderDialog: true,
  ...
  const configResponse = await request.get(`${apiURL}/config`);
...
test("csv import replaces existing rows", async ({
  ...
  const importTwo = await request.post(`${apiURL}/bookings/import`, {
    headers: {
      "Content-Type": "text/csv; charset=utf-8",
    },
    data: csvTwo,
