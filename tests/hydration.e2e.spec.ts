...
import { expect, test, type APIRequestContext, type Page } from "./helpers/config-fixtures";
...
async function getTodayMl(request: APIRequestContext, apiBase: string) {
...
test.describe("Trinkmanager", () => {
  test("startet bei 0 ml und schreibt nach einem Klick die Tagessumme in die Datei", async ({
    ...
    await expect
      ...
      .toBe(250);
    await expect.poll(async () => getTodayMl(request, apiURL)).toBe(250);
  });
  ...
  test("geht nie ins Minus: Button ist bei 0 ml deaktiviert", async ({ page, appURL }) => {
    await page.goto(appURL);
    expect(await readDisplayedMl(page)).toBe(0);
  ...
  test("Randfall: mehrere Tageseinträge summieren sich nicht auf, nur der letzte Stand pro Tag zählt", async ({
    ...
    const response = await request.get(`${apiURL}/dashboard?limit=1`);
    expect(response.ok()).toBeTruthy();
